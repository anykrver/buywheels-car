...
export default function VehicleFilters({ filters, onChange, brands }: VehicleFiltersProps) {
  const [expandedSections, setExpandedSections] = useState({ brands: true, fuel: true, transmission: true, price: true, safety: true });
  ...
  const clearAll = () => {
    onChange({ brands: [], fuelTypes: [], transmissions: [], priceMin: 0, priceMax: 100000000, sortBy: 'popularity', safetyRating: undefined });
  };

  const hasActive = filters.brands.length > 0 || filters.fuelTypes.length > 0 || filters.transmissions.length > 0 || !!filters.safetyRating;
  ...
  return (
    <div className="bg-white rounded-2xl border border-border shadow-card">
      ...
      <div className="p-5">
        ...
        >
          <span className="text-xs font-semibold text-muted uppercase tracking-wider">Safety Rating</span>
          {expandedSections.safety ? <ChevronUp size={14} className="text-muted" /> : <ChevronDown size={14} className="text-muted" />}
        </button>
            ...
            {[3, 4, 5].map(stars => (
              <label key={stars} className="flex items-center gap-3 cursor-pointer group">
                <input
                  type="radio"
                  name="safety"
                  checked={filters.safetyRating === stars}
                  onChange={() => onChange({ ...filters, safetyRating: filters.safetyRating === stars ? undefined : stars })}
                  className="accent-primary w-4 h-4"
                />
                <span className="flex items-center gap-1.5 text-sm text-dark-600 group-hover:text-primary transition-colors">
                  {Array.from({ length: stars }).map((_, i) => (
                    <Star key={i} size={13} className="fill-warning text-warning" />
                  ))}
                  {stars < 5 && <span className="text-muted">& above</span>}
                  {stars === 5 && <span className="font-semibold text-success">5 Star</span>}
                </span>
              </label>
            ))}
            {filters.safetyRating && (
              <button
                onClick={() => onChange({ ...filters, safetyRating: undefined })}
                className="text-xs text-primary hover:text-primary-600 flex items-center gap-1 mt-1"
              >
                <X size={11} /> Clear safety filter
              </button>
            )}
          </div>
        )}
