...
import VehicleFilters, { Filters } from '../components/vehicle/VehicleFilters';
...
export default function VehicleListing({ category, title, subtitle }: VehicleListingProps) {
  ...
  const filtered = useMemo(() => {
    ...
    let result = uniqueCategoryVehicles.filter(v => {
      ...
      // 10. Safety rating filter
      if (filters.safetyRating) {
        const sr = v.safetyRating || 0;
        if (sr < filters.safetyRating) return false;
      }
