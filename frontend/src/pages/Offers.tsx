...
import {
  ...
  Heart, ShieldCheck, Zap, Building2, Landmark, RefreshCw, Percent, Search, Gift, Flame
} from 'lucide-react';
...
// ──────────────────────────────────────────────────
// Countdown Timer Component for deals ending soon (PRD §7.5.3)
// ──────────────────────────────────────────────────
function OfferCountdown({ validTill }: { validTill: string }) {
  const [timeLeft, setTimeLeft] = useState<{ days: number; hours: number; minutes: number; seconds: number } | null>(null);

  useEffect(() => {
    // Parse validTill or default to 5 days from now for demo
    let targetDate = new Date(validTill).getTime();
    if (isNaN(targetDate)) {
      targetDate = Date.now() + (5 * 24 * 60 * 60 * 1000) + (14 * 60 * 60 * 1000);
    }
  ...
  return (
    <div className="bg-red-50 border border-red-200 text-red-600 rounded-xl px-3 py-1.5 flex items-center justify-between text-xs font-semibold mb-4">
  ...
  return (
    <div className="group bg-white rounded-2xl border border-border shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1 overflow-hidden flex flex-col h-full">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        {/* Live Countdown Timer if ending soon (PRD §7.5.3) */}
        {offer.validTill && <OfferCountdown validTill={offer.validTill} />}
...
export default function Offers() {
  ...
  return (
    <div className="min-h-screen bg-surface pt-20 pb-20 lg:pb-12">
      <PageSEO
        title="Best Car Deals & Festival Offers in Jharkhand | Discount Coupon Codes | BuyWheels"
        description="Explore verified bank cashbacks, exchange bonuses, zero down payment schemes, and dealership discount coupons for Maruti, Hyundai, Tata & Mahindra in Jharkhand."
